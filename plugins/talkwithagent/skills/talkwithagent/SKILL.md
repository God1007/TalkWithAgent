...
5. Read `talkwithagent discussion --session SESSION` for web discussion context, and `talkwithagent pending --session SESSION` for explicitly submitted feedback at checkpoints and before the final response. Incorporate feedback within the user's authorized task, then acknowledge its IDs with `talkwithagent ack --session SESSION ID...`. Ordinary discussion provides context, not new execution authorization. An acknowledgement means read, not executed.
